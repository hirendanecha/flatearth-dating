...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/flatearth-dating/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'dist/flatearth-dating/browser'),
        'index.html'
      )
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'flatearth-dating Post',
          ...
          const talent = {
            name: `FlatEarth.dating Research ${group?.PageTitle}`,
